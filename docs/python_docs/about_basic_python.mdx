...
* **동작 방식:**
    ...
    2.  `@my_decorator`를 함수 `func` 위에 쓰면, 파이썬은 내부적으로 `func = my_decorator(func)` 와 같이 처리합니다. 즉, 원래 함수 `func`가 데코레이터 함수 `my_decorator`에 전달되고, 그 반환값(주로 내부 wrapper 함수)이 다시 `func`라는 이름에 할당됩니다.
    3.  이후 `func()`를 호출하면 실제로는 데코레이터가 반환한 함수가 실행됩니다. 이 반환된 함수는 보통 내부에서 원본 함수를 호출하면서 추가적인 로직(전처리, 후처리)을 수행합니다.
* **특징 및 용도:**
...
async def say_after(delay, what):
  await asyncio.sleep(delay) # delay 초 동안 대기 (다른 작업 실행 가능)
  print(what)
...
이 개념들을 이해하는 것은 효율적이고 성능 좋은 파이썬 코드를 작성하는 데 매우 중요합니다. 궁금한 점이 있다면 언제든지 다시 질문해주세요.